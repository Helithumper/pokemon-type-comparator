...
def main():
	while True:
		...
		if len(typelower.split())==1:
			typeindex = typeTable.index(typelower);
			#print(typeindex);

			for j in range(0,17):
				if matchupTable[j][typeindex] == 2:
					print(typeTable[j].upper());
		elif len(typelower.split())==2:
			typeindex1 = typeTable.index(typelower.split()[0]);
			typeindex2 = typeTable.index(typelower.split()[1]);
			
			weaks = [];
			powerfuls = [];

			for j in range(0,17):
				if matchupTable[j][typeindex1] == 2:
					if(j not in powerfuls):
						powerfuls.append(j);
				if matchupTable[j][typeindex1] == 0 or matchupTable[j][typeindex1] == 3:
					if(j not in weaks):
						weaks.append(j);
				if matchupTable[j][typeindex2] == 2:
					if(j not in powerfuls):
						powerfuls.append(j);
				if matchupTable[j][typeindex2] == 0 or matchupTable[j][typeindex2] == 3:
					if(j not in weaks):
						weaks.append(j);
